...
if __name__ == '__main__':
    ...
    model_RepUNet = torch.load('RepUNet.pkl')
    model_RepUNet.to(DEVICE)
    for module in model_RepUNet.modules():
    ...
    for i in range(len(path)):
        ...
        img_repunet = cv2.split(img)[1]
        img_repunet = np.transpose(np.expand_dims(np.expand_dims(img_repunet, axis=-1), axis=0), (0, 3, 1, 2))
        img_repunet = torch.from_numpy(img_repunet).to(DEVICE).float()
        pred_repunet = np.argmax(model_RepUNet(img_repunet).cpu().detach().numpy()[0], axis=0).reshape((-1))
        ...
        repunet_pa, repunet_mpa, repunet_miou = metrice(mask_true, pred_repunet)
        ...
        pred_repunet = np.array(pred_repunet, dtype=np.uint8).reshape((256, 256))
        pred_repunet_img = cv2.resize(pred_repunet, (img_shape[1], img_shape[0]), interpolation=cv2.INTER_NEAREST)

        pred_unet_img[pred_unet_img == 1] = 255
        pred_repunet_img[pred_repunet_img == 1] = 255
        ...
        plt.subplot(2, 2, 2)
        plt.bar(np.arange(3), [unet_pa, unet_mpa, unet_miou], 0.3, label='unet', tick_label=['pa', 'mpa', 'miou'])
        plt.bar(np.arange(3) + 0.3, [repunet_pa, repunet_mpa, repunet_miou], 0.3, label='repunet', tick_label=['pa', 'mpa', 'miou'])
        plt.legend()
        ...
        plt.subplot(2, 2, 4)
        plt.imshow(pred_repunet_img, cmap=plt.cm.gray)
        plt.title('repunet-pred\npa:{:.3f} mpa:{:.3f} miou:{:.3f}'.format(repunet_pa, repunet_mpa, repunet_miou))
        plt.axis('off')
